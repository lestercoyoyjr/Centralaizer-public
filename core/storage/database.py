...
-- forget-verification audit trail: proof that a memory was erased across every
-- store, and a record of any store that still had it (a "leak").
CREATE TABLE IF NOT EXISTS forgotten (
    id           TEXT PRIMARY KEY,
    forgotten_at TEXT NOT NULL DEFAULT (strftime('%Y-%m-%dT%H:%M:%fZ','now')),
    reason       TEXT NOT NULL DEFAULT 'decay',
    verified     INTEGER NOT NULL DEFAULT 0,   -- 1 = confirmed gone from all stores
    leaks        TEXT NOT NULL DEFAULT '[]',   -- JSON list of stores that still had it
    cascaded     TEXT NOT NULL DEFAULT '[]',   -- JSON list of derived memories flagged stale (id-keyed)
    semantic     TEXT NOT NULL DEFAULT '[]'    -- JSON list of memories flagged for review (semantic match)
);
...
def init_db() -> None:
    ...
    for stmt in ("ALTER TABLE forgotten ADD COLUMN cascaded TEXT NOT NULL DEFAULT '[]'",
                 "ALTER TABLE forgotten ADD COLUMN semantic TEXT NOT NULL DEFAULT '[]'"):
        try:
            conn.execute(stmt)
