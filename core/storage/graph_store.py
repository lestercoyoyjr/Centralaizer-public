...
def has(memory_id: str) -> bool:
    """True if any edge still references the id — for forget-verification."""
    with _conn() as con:
        return con.execute(
            "SELECT 1 FROM edges WHERE src=? OR dst=? LIMIT 1", [memory_id, memory_id]
