...
def _manager_tick():
    ...
    # sweep newly-ended Claude Code sessions + sync changed native memory files into
    # the hub. Catches sessions closed abruptly (terminal/tab killed, which never fire
    # the SessionEnd hook) and Claude Code's own per-project memory markdown. Both are
    # incremental (watermark-gated), so this is cheap to run every tick.
    for args in (["--sweep"], ["--sync-native"], ["--sweep-editors"]):
        try:
            subprocess.run([sys.executable, _HOOK, *args], timeout=300, check=False)
        except Exception as e:
            log.warning("session %s failed: %s", args[0], e)
