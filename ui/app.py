...
from core.memory.engine import (
    write_memory, search_memory, delete_memory, forget_memory,
    approve_quarantine, reject_quarantine,
    list_memories, list_quarantine, list_review_queue, clear_review_flag, get_stats,
    count_quarantine, count_review_queue,
...
def render(name: str, ctx: dict) -> HTMLResponse:
    # sidebar badge counts, available on every page (route ctx wins if it sets them).
    # exact COUNT queries — not len(list_…()), which caps at the list LIMIT and would
    # diverge from the dashboard card once a queue exceeds it.
    base = {"quarantine_count": count_quarantine(), "review_count": count_review_queue()}
    base.update(ctx)
    tmpl = _jinja_env.get_template(name)
    return HTMLResponse(tmpl.render(**base))
...
async def review_page(request: Request):
    entries = list_review_queue(limit=200)
    return render("review.html", {"entries": entries, "page": "review"})
...
@app.post("/api/memories/{memory_id}/forget")
async def api_forget(memory_id: str):
    """Erase a flagged memory (confirmed leak) with full verification + cascade."""
    return forget_memory(memory_id, reason="gdpr-review")

@app.post("/api/memories/{memory_id}/clear-flag")
async def api_clear_flag(memory_id: str):
    """Dismiss a false-positive review flag; keep the memory."""
    if not clear_review_flag(memory_id):
