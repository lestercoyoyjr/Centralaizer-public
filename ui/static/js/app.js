...
// keep the sidebar badge + header count in sync when a review row leaves the queue
function decrementReview() {
  const b = document.getElementById('review-badge');
  if (b) { const n = (parseInt(b.textContent, 10) || 1) - 1; n <= 0 ? b.remove() : (b.textContent = n); }
  const h = document.getElementById('review-count-hdr');
  if (h) h.textContent = Math.max(0, (parseInt(h.textContent, 10) || 1) - 1);
}

async function forgetFlagged(id, rowEl) {
  if (!confirm('Permanently erase this memory from all stores?')) return;
  try {
    const r = await api('POST', `/api/memories/${id}/forget`);
    rowEl.style.opacity = '0';
    setTimeout(() => rowEl.remove(), 300);
    decrementReview();
    toast(r.verified ? 'Erased and verified ✓' : `Erased — leaks: ${(r.leaks||[]).join(', ')}`, !r.verified);
    refreshStats();
  } catch(e) { toast(e.message, true); }
}

async function dismissFlag(id, rowEl) {
  try {
    await api('POST', `/api/memories/${id}/clear-flag`);
    rowEl.style.opacity = '0';
    setTimeout(() => rowEl.remove(), 300);
    decrementReview();
